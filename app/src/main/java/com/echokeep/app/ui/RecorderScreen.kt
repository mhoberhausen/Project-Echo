...
fun RecorderScreen(
    ...
    Surface(modifier = Modifier.fillMaxSize()) {
        Column(
            ...
            when (state.phase) {
                RecordingPhase.IDLE -> Button(onClick = onRecord) { Text("Start listening") }
                ...
                RecordingPhase.PROCESSING -> CircularProgressIndicator()
                RecordingPhase.COMPLETE -> TranscriptResult(state, onRecord, onClear)
...
@Composable
private fun TranscriptResult(state: RecorderUiState, onRecord: () -> Unit, onClear: () -> Unit) {
