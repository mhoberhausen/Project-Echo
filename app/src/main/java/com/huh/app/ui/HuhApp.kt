...
private enum class AppDestination { HOME, MANUAL, LIVE, SESSION, PREFERENCES, ADVANCED }
...
fun HuhApp(
    ...
    val returnHome: () -> Unit = {
        if (destination == AppDestination.MANUAL && recorderState.phase == com.huh.app.model.RecordingPhase.RECORDING) {
            onCancelRecording()
        }
    ...
    val navigateBack: () -> Unit = {
        if (destination == AppDestination.ADVANCED) destination = AppDestination.PREFERENCES
        else returnHome()
    }
    ...
    Column(
        ...
        Text(
            when (snapshot.state) {
                ...
                ActiveListeningState.STARTING -> "Starting the microphone on this device."
                ActiveListeningState.WAITING -> "Listening for conversations on this device."
